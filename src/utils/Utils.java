...
/*
    ...
    public static void processDataFlow(String attribute, String inputData){
         Map<String, Counter> optionsMap = createMapForAttribute(attribute);
         String [] optionDecisionPairs = inputData.split(" ");

        for(String opDec : optionDecisionPairs){

            String [] tokens  = opDec.split(TrainingReducer.DELIMITER);
            String option = tokens[0];
            String decision = tokens[1];

            countNumberOfOpt(optionsMap, option, decision);
        }
    ...
    private static Map<String, Counter> createMapForAttribute(String attribute ){

        Map<String, Counter> smallMap = new HashMap<String, Counter>();
        CLASSES.put(attribute, smallMap);
        return smallMap;
    }



    private static  void countNumberOfOpt(Map<String, Counter> map, String keyOption, String decision){

        Counter counter = map.get(keyOption);

        if(counter != null){
            //update counter
            updateCounter(counter, decision);
        }
        else {
            //increment counter
            initCounter(map, keyOption, decision);
        }
    }

    private static void updateCounter(Counter counter, String decision){

        if(decision.equals("yes"))
            countPositiveDecision(counter);
        else
            countNegativeDecision(counter);
    }

    private static void initCounter(Map<String,Counter> map, String option, String decision){
        Counter counter = new Counter();
        //countDecision(counter, decision);
        if(decision.equals("yes"))
            countPositiveDecision(counter);
            ...
            countNegativeDecision(counter);
        map.put(option, counter);
    }


    public static void countPositiveDecision(Counter counter){
            counter.countYes++;
            counter.countAttribute++;

    }

    public static void countNegativeDecision(Counter counter){
        counter.countNo++;
        counter.countAttribute++;
